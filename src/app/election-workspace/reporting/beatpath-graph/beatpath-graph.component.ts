  ...
  */
  ...
  ngOnInit() {
    ...
    this.reporting_data_update_sub = this.reporting_service.report_data_updated
        .subscribe(
            (report_data: { pref_strengths:[][], labels: String[], winner: String[] }) => {
  ...
  draw_grid(input_data: canvas_data){
    ...
    for (let i = 0; i < input_data.pairwise_data.length; i++) {
      this.ia_diagram_labels.push(alphabet.charAt(i));
    }
  ...
  tableCellRedOrGreen(rowIndex: number, columnIndex: number) {
    ...
    let cell1 = this.ia_pref_strengths[rowIndex][columnIndex],
